...
const ReportList = () => {
  ...
  const fetchData = async (page, pageSize) => {
    try {
      const response = await fetch(`${API_URL}/admin/report/?limit=${pageSize}&pageNo=${page + 1}`);
      ...
      const formattedData = data.results.map((item) => ({
        ...
        created_at: new Date(item.reported_date).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        }),
  ...
  const columns = [
...

    {
        ...
        renderCell: (params) => (
            <Tooltip 
            title={<Typography sx={{ fontSize: '14px' }}>{params.value}</Typography>} 
            arrow
          >
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (params) => (
        <Link to={`/reporteduserdetails/${params.row.id}`} style={{ color: colors.blueAccent[400], textDecoration: 'none' }}>
          Show
        </Link>
      ),
    },
  ...
  return (
    <Box m="20px">
      <Header
        mb={2}
        title="Reports List"
        subtitle="List of reported users for future reference"
      />

...
const RejectedList = () => {
  ...
  const fetchData = async (page, pageSize) => {
    try {
      const response = await fetch(`${API_URL}/admin/rejected/customers?limit=${pageSize}&pageNo=${page + 1}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (params) => (
        <Link to={`/rejected/${params.row.id}`} style={{ color: colors.blueAccent[400], textDecoration: 'none' }}>
          Show
        </Link>
      ),
    },
  ...
  return (
    <Box m="20px">
      <Header
        mb={2}
        title="Rejected List"
        subtitle="List of rejected users for future reference"
      />

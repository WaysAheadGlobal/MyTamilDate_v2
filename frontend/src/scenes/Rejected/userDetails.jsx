...
const RejectDetails = () => {
  ...
  const detailsfetchData = async () => {
    try {
      const response = await fetch(`${API_URL}/admin/rejected/approval/${id}`);
      const data = await response.json();
      setDetails(data);
    } catch (err) {
      console.log(err);
  ...
  const rejectreasionfetch = async()=>{
    try {
        const response = await fetch(`${API_URL}/admin/rejected/latestrejection/${id}`);
        const data = await response.json();
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Box display="flex" flexDirection="column" alignItems="center" mb={2} mt={2}>
            ...
            <Typography variant="subtitle1" color="textSecondary" align="center">{formatValue('status', details.status === 30 ? "Reject" : "Rejected")}</Typography>
          </Box>
        </Grid>
        <Grid item xs={12} md={8}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
                <Box mb={4}>
                  <Typography variant="h4" gutterBottom sx={{ color: colors.primary[500] }}>Reject Reason</Typography>
                    <Box  mb={2}>
                      <Typography fontWeight="bold" variant="h6">{RejectReason.reason}</Typography>
                      {/* <Typography variant="body1">{item.answer}</Typography> */}
                    </Box>

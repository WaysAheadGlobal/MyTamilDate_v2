...
const getDateConditionage = (timeRange) => {
    let condition = '';
    switch (timeRange) {
        case '24h':
...
const getDateConditionoflike = (timeRange) => {
    let condition = '';
    switch (timeRange) {
        case '24h':
...
const getDateConditionMessage = (timeRange) => {
    let condition = '';
    switch (timeRange) {
        case '24h':
...
const getDateConditionpayment = (timeRange) => {
    switch (timeRange) {
        case '24h':
...
const getDateConditionJobs = (timeRange) => {
    let condition = '';
    switch (timeRange) {
        case '24h':
...
const getUserCreationDateCondition = (timeRange) => {
    switch (timeRange) {
        case '24h':
...
const getDateConditionlocations = (timeRange, tableAlias = '') => {
    let condition = '';
...
dashboard.get('/messages/count', (req, res) => {
...
dashboard.get('/likes/count', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
...
dashboard.get('/matches/count', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
    const dateCondition = getDateCondition(timeRange);
    const sql = `
...
dashboard.get('/request/count', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
    const dateCondition = getDateConditionMessage(timeRange);
    const sql = `SELECT COUNT(*) AS total_messages FROM dncm_messages WHERE ${dateCondition}`;
    console.log('Executing SQL:', sql);
...
dashboard.get('/top-countries', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
...
//     });
// });
dashboard.get('/count-by-gender', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
    const dateCondition = getDateCondition(timeRange);
    const sql = `
        SELECT
    ...
    db_1.db.query(sql, (err, results) => {
        ...
        const countByGender = results.reduce((acc, row) => {
...
dashboard.get('/top-jobs', (req, res) => {
    ...
    const timeRange = req.query.timeRange || '24h'; // Default to 24 hours if not provided
    const dateCondition = getDateConditionJobs(timeRange);
    const sql = `
    ...
    `;
    console.log(sql);
    db_1.db.query(sql, [limit], (err, results) => {
        if (err) {
            console.error('Error fetching data:', err);
            res.status(500).json({ error: 'Internal Server Error' });
            return;
...
//     });
// });
dashboard.get('/users/age-group', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
...
dashboard.get('/users/count', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
    const dateCondition = getDateCondition(timeRange);
...
dashboard.get('/payments', (req, res) => {
    const sql = `
...
//     });
// });
dashboard.get('/payments/total', (req, res) => {
    const timeRange = req.query.timeRange || '24h';
...
dashboard.get('/renewals/count', (req, res) => {
    ...
    console.log('Executing SQL:', sql);
    db_1.db.query(sql, [process.env.CASHIER_PRICE_REGULAR], (err, results) => {
        if (err) {
            console.error('Error fetching data:', err);
            res.status(500).json({ error: 'Internal Server Error' });
            return;
...
dashboard.get('/new-paid-members/count', (req, res) => {
    ...
    db_1.db.query(sql, (err, results) => {
        ...
        const totalNewPaidMembers = ((_a = results[0]) === null || _a === void 0 ? void 0 : _a.total_new_paid_members) || 0;
...
dashboard.get('/requests/count', (req, res) => {
    ...
    db_1.db.query(sql, (err, results) => {
        ...
        const totalRequests = ((_a = results[0]) === null || _a === void 0 ? void 0 : _a.total_requests) || 0;
...
`;
    ...
    db_1.db.query(sql, (err, results) => {
        ...
        const totalOldMembersSignedIn = ((_a = results[0]) === null || _a === void 0 ? void 0 : _a.total_old_members_signed_in) || 0;

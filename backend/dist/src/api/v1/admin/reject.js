...
const reject = (0, express_1.Router)();
...
reject.get('/approval/:user_id', (req, res) => {
...
reject.get('/latestrejection/:user_id', (req, res) => {
    const userId = req.params.user_id;
    const query = `
...
reject.post('/updateRejected', (req, res) => {
    ...
    db_1.db.query(getUserEmailSql, [id], (err, results) => {
      ...
      `;
        db_1.db.query(insertRejectReasonSql, [message], (err, result) => {
          ...
          VALUES (?, ?, NOW(), NOW(), 1)
        `;
            db_1.db.query(insertRejectSql, [id, reasonId], (err) => {
                if (err) {
